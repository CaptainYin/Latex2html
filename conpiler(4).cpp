...
public:
	stack<int> state;
	stack<actiontype>List;
	int pos;
	
	Analysis(stack<int> s,stack<actiontype>L,int p,string t);
	~Analysis();
	int Analysis::generateActList(const SLR_Analysis_Table SLR_T);//1:acc,0:fork,-1:error
private:
	string text;
	int ind_ter;
	void Analysis::updateind_ter();
};
int Analysis::generateActList(const SLR_Analysis_Table SLR_T){
	const actiontype** ActionTable=SLR_T.ActionTable;
	const int *Tansfer_table = SLR_T.Tansfer_table;
	while (true)
	{
		updateind_ter();
		...
		else
		{
			...
			if (ActionTable[currentState][ind_ter].action == 'a')return 1;//acc
			if (ActionTable[currentState][ind_ter].action == 's')
			{
				state.push(ActionTable[currentState][ind_ter].state);
			}
			else
			{//reduce
				size_t length = syntax[ActionTable[currentState][ind_ter].ind].length();
				for (size_t i = 0; i < length; i++)
				{
					state.pop();
					if (state.empty())cout << "err:pop too much" << endl;
				}
				int toState = Tansfer_table[state.top()];
				state.push(toState);
			}
			pos++;
			if (pos==text.length())
...
Analysis::Analysis(stack<int> s, stack<actiontype>L, int p, string t)
{
	List = L;
	state = s;
	pos = p;
